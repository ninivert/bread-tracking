...
def plot_assignmentgraph_track(img_seg1, img_seg2, g1, g2):
	...
	fig, ax = plot_assignmentgraph(
		g1, g2, gA,
		kwargs_g1=dict(pos=bread.utils.get_cms(img_seg1)[:, [1, 0]]),
		kwargs_g2=dict(pos=bread.utils.get_cms(img_seg2)[:, [1, 0]]),
...
def plot_assignmentgraph(g1, g2, gA, kwargs_g1={}, kwargs_g2={}, kwargs_gA={}):
	...
	ax[2].set_title('$G^{(A)}$')
	plt.sca(ax[0])
	bread.vis.graph.plot_graph_networkx(g1, labels=labels1, **kwargs_g1)
	plt.sca(ax[1])
	bread.vis.graph.plot_graph_networkx(g2, labels=labels2, **kwargs_g2)
	plt.sca(ax[2])
	bread.vis.graph.plot_graph_networkx(gA, labels=labelsA, **kwargs_gA)
	fig.tight_layout()
