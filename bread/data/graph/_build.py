...
def build_cellgraph(img_seg, max_membdist=12, undirected=False, return_df=False):
	...
	df_x = bread.data.features.extract_features_cells(img_seg)

	# Edge features
	# we could simply ask to compute all the pairwise features, but that's silly and scales badly

	cell_ids = bread.utils.get_cellids(img_seg)  # slice to ignore background segmentation, which has id=0
	idmap = bread.utils.cellid_to_nodeid(cell_ids)

	contours = []
	for cell_id in cell_ids:
		contours.append(bread.utils.cell_contour(img_seg == cell_id))
	...
	df_e = bread.data.features.extract_features_pairs(img_seg, pairs)
...
def build_assignmentgraph_track(img_seg1, img_seg2) -> Data:
	...
	cell_ids2 = bread.utils.get_cellids(img_seg2)
