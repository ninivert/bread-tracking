...
class AssignmentGraphDataset(Dataset):
	...
	def __init__(self, filepaths, preload=False, transform=None):
		...
		if preload:
			self.graph_cache = [bread.data.graph.from_npz(bread.data.utils.load_npz(self.filepaths[idx])) for idx in range(len(self))]
	...
	def __getitem__(self, idx):
		if self.preload:
			...
			graph = bread.data.graph.from_npz(bread.data.utils.load_npz(self.filepaths[idx]))
